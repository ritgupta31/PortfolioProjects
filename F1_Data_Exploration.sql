...
(SELECT
	races.name, races.year, drivers.driverId, CONCAT(drivers.forename, ' ', drivers.surname) AS driver, results.grid, results.points, results.position, results.rank
FROM
	results
INNER JOIN
	drivers
...
SELECT
	year, driver, sum(convert(points, signed)) AS TotalPoints, RANK() OVER (PARTITION BY year ORDER BY sum(convert(points, signed)) DESC) AS ChampionshipRank
FROM
	raceresults
GROUP BY
	driver, year
...
SELECT 
	a.driver, count(a.ChampionshipRank) AS ChampionshipWins
FROM
	(
(SELECT
	year, driver, sum(convert(points, signed)) AS TotalPoints, RANK() OVER (PARTITION BY year ORDER BY sum(convert(points, signed)) DESC) AS ChampionshipRank
...
FROM
(SELECT
	races.year, races.name, circuits.name AS circuit , circuits.location, circuits.country, CONCAT(drivers.forename, ' ', drivers.surname) AS driver, results.points, results.position
FROM
	results
INNER JOIN
	drivers
...
WHERE
	results.position = 1
ORDER BY 
	races.year DESC, races.name) AS a
...
FROM
	(SELECT a.circuit, a.location, a.country, a.driver, count(a.driver) AS TimesWon
	FROM
		(SELECT circuits.name AS circuit, circuits.location, circuits.country, CONCAT(drivers.forename, ' ', drivers.surname) AS driver, results.position
		FROM results
		INNER JOIN 
			drivers
		...
		ON races.circuitId = circuits.circuitId
		WHERE
			results.position = 1) AS a
	GROUP BY a.circuit, a.driver, a.location, a.country
	ORDER BY a.country, TimesWon DESC
    ) AS b
    INNER JOIN
		(SELECT circuits.name AS circuit , circuits.location, CONCAT(drivers.forename, ' ', drivers.surname) AS driver, count(drivers.driverId) AS TimesRaced
		FROM results
		INNER JOIN
			drivers
		ON results.driverId = drivers.driverId
		...
		ON races.circuitId = circuits.circuitId
		GROUP BY 
			circuits.name, circuits.location, CONCAT(drivers.forename, ' ', drivers.surname)
...
SELECT
	circuits.name AS circuit , circuits.location, circuits.country, CONCAT(drivers.forename, ' ', drivers.surname) AS driver, count(drivers.driverId) AS TimesRaced
FROM
	results
INNER JOIN
	drivers
...
ON races.circuitId = circuits.circuitId
GROUP BY 
	circuits.name, circuits.location, circuits.country, CONCAT(drivers.forename, ' ', drivers.surname)
ORDER BY 
	TimesRaced DESC;
...
FROM
(SELECT
	races.year, races.name, circuits.name AS circuit , circuits.location, circuits.country, results.fastestLapTime
FROM
	results
INNER JOIN
	races
...
ON
	races.circuitId = circuits.circuitId
WHERE
	results.fastestLapTime NOT LIKE '%N%'
ORDER BY 
	races.year DESC, races.name) AS a
GROUP BY
	a.name, a.circuit, a.location
ORDER BY 
	a.name, a.location;
...
SELECT 
	races.year, races.date, races.name, constructors.name, constructor_results.points
FROM 
	races
INNER JOIN
	constructor_results
ON
	races.raceId = constructor_results.raceId
INNER JOIN
	constructors
ON
	constructor_results.constructorId = constructors.constructorId
ORDER BY
	races.date DESC, points DESC;
...
(
SELECT 
	races.year AS year, constructors.name, sum(constructor_results.points) AS points, RANK() OVER (PARTITION BY year ORDER BY sum(constructor_results.points) DESC)
FROM 
	races
INNER JOIN
	constructor_results
